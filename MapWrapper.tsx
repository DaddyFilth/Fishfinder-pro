...
function waterLevelLabel(level: number | null, flow: number | null) {
  if (level !== null) {
    return `Water level: ${level.toFixed(2)} m`;
  }
  if (flow !== null) {
    return `Flow: ${flow.toFixed(0)} cfs`;
  }
  return 'No water-level data';
}
                  ...
                  <div style={{ ...S.popupWrap, minWidth: popupFrame.minWidth, maxHeight: popupFrame.maxHeight }}>
                    ...
                    {c ? (
                      <div style={{ background: '#082f49', border: '1px solid #155e75', borderRadius: 10, padding: 10, marginBottom: 10 }}>
                        <div style={{ fontSize: 10, color: '#67e8f9', fontWeight: 800, marginBottom: 7 }}>CATALOG TARGET ESTIMATE</div>
                        ...
                        <div style={{ color: '#94a3b8', fontSize: 9, marginTop: 7 }}>Rule-based catalog estimate using the returned conditions and spot type; not an AI determination.</div>
                      </div>
                    ) : (
                      <div style={{ background: '#172033', border: '1px solid #334155', borderRadius: 10, padding: 10, marginBottom: 10, color: '#fbbf24', fontSize: 10 }}>
                        Live conditions unavailable. No target score is shown.
                      </div>
                    )}
                            ...
                            >
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <div style={{ textAlign: 'right', fontSize: 11, color: '#cbd5e1' }}>
                                  <div>{waterLevelLabel(c.water_level_m, c.flow_rate_cfs)}</div>
